...
public:
    ...
    void onKey(int key, int scanCode, int action, int mods) override {
        ...
        if (keys[GLFW_KEY_A]) {
            //std::cout << "viewMatrix " << glm::to_string(camera.viewMatrix) << " " << camera.viewMatrix[0][0];
            glm::vec3 eyePos = camera.viewMatrix[0];
            eyePos[0] -= 0.1;
            //camera.viewMatrix = glm::lookAt(eyePos, camera.viewMatrix[1], camera.viewMatrix[2], camera.viewMatrix[3]);
            camera.viewMatrix[0][0] -= 0.1;
        }
        if (keys[GLFW_KEY_D]) {
            //std::cout << "viewMatrix " << glm::to_string(camera.viewMatrix) << " " << camera.viewMatrix[0][0];
            glm::vec3 eyePos = camera.viewMatrix[0];
